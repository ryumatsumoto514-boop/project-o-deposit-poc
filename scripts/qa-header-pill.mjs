// One-off QA check: verify the connected-wallet header pill no longer
// forces the checksummed address into uppercase (defeats EIP-55 visual
// checksum). Mirrors shot-flow-walk.mjs's connect pattern.
const CDP = "http://127.0.0.1:9333";
const BASE_URL = process.argv[2];
const OUT_DIR = process.argv[3];
const WALLET = "0x9a11f5B6D8c2A7e4C1d3F0b8E6a9C4d7F2b1A3e5"; // mixed-case on purpose
...
fs.writeFileSync(`${OUT_DIR}/header-pill-check.png`, Buffer.from(data, "base64"));
console.log(`wrote ${OUT_DIR}/header-pill-check.png`);

// Also pull the rendered text content directly, not just pixels.
const { result: pillText } = await send(ws, "Runtime.evaluate", {
  expression: `document.querySelector('[aria-label^="Funding wallet"]')?.textContent`,
  returnByValue: true,
});
console.log("header pill text:", pillText.value);
