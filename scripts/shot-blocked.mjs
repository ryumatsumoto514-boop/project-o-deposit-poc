// Single continuous CDP session that click-walks the REAL flow (connect ->
// amount -> confirm -> approve) so wagmi's in-memory connection state
// survives client-side navigation, then screenshots the duplicate-blocked
// screen. Requires an in-flight deposit already created for WALLET+AMOUNT
// (see the curl command logged alongside this script's invocation).
  ...
  source: `
    (function() {
      ...
      sessionStorage.setItem("exo_flow_state", JSON.stringify({
        ...
        draftAmount: ${JSON.stringify(AMOUNT)},
        approvalMode: "exact",
...
await wait(1200);
console.log("clicking Connect Injected (if not already auto-reconnected) ...");
try {
  await clickText(ws, "Connect Injected");
} catch {
  console.log("  already connected (wagmi auto-reconnected from this profile's localStorage) — skipping");
}
await wait(800);
console.log("clicking Continue (amount) ...");
await clickText(ws, "Continue");
await wait(900);
console.log("checking confirm checkbox ...");
...
await wait(300);
console.log("clicking Continue (confirm) ...");
await clickText(ws, "Continue");
await wait(900);
console.log("clicking Approve & deposit ...");
await clickText(ws, "Approve & deposit");
await wait(1500);
